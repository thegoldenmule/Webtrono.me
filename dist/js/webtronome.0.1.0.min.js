...
var __buildTimestamp="140924171750-0500";"use strict";function Daemon(oOwner,fTask,nRate,nLen,fInit,fOnstart){if(!(this&&this instanceof Daemon)){return;}
if(arguments.length<2){throw new TypeError("Daemon - not enough arguments");}
if(oOwner){this.owner=oOwner};this.task=fTask;if(isFinite(nRate)&&nRate>0){this.rate=Math.floor(nRate);}
if(nLen>0){this.length=Math.floor(nLen);}
if(fOnstart){this.onstart=fOnstart;}
if(fInit){this.onstop=fInit;fInit.call(oOwner,this.INDEX,this.length,this.BACKW);}}
Daemon.blank=function(){};Daemon.context=Daemon.blank.prototype;Daemon.blank.prototype=Daemon.prototype;Daemon.context.constructor=Object;Daemon.prototype.owner=Daemon.context;Daemon.prototype.task=null;Daemon.prototype.rate=100;Daemon.prototype.length=Infinity;Daemon.prototype.reversals=0;Daemon.prototype.onstart=null;Daemon.prototype.onstop=null;Daemon.prototype.SESSION=-1;Daemon.prototype.INDEX=0;Daemon.prototype.PAUSED=true;Daemon.prototype.BACKW=true;Daemon.forceCall=function(oDmn){oDmn.INDEX+=oDmn.BACKW?-1:1;var
bBreak=oDmn.task.call(oDmn.owner,oDmn.INDEX,oDmn.length,oDmn.BACKW)===false,bEnd=oDmn.isAtEnd(),bInvert=oDmn.reversals>0;if(bEnd&&!bInvert||bBreak){oDmn.pause();return false;}
if(bEnd&&bInvert){oDmn.BACKW=!oDmn.BACKW;oDmn.reversals--;}
return true;};Daemon.construct=function(aArgs){var oDmn=new this.blank();this.apply(oDmn,aArgs);return oDmn;};Daemon.buildAround=function(oCtx,nRate,nLen){if(!oCtx.perform){throw new TypeError("You cannot create a daemon around an object devoid of a \"perform\" function");}
return new this(oCtx,oCtx.perform,nRate||null,nLen||null,oCtx.create||null,oCtx.prepare||null);};Daemon.incorporate=function(fConstr){var oLegacy=fConstr.prototype;fConstr.prototype=new Daemon.blank();fConstr.prototype.legacy=oLegacy;return fConstr;};Daemon.prototype.isAtEnd=function(){return this.BACKW?isFinite(this.length)&&this.INDEX<1:this.INDEX+1>this.length;};Daemon.prototype.synchronize=function(){if(this.PAUSED){return;}
clearInterval(this.SESSION);this.SESSION=setInterval(Daemon.forceCall,this.rate,this);};Daemon.prototype.pause=function(){clearInterval(this.SESSION);this.PAUSED=true;};Daemon.prototype.start=function(bReverse){var bBackw=Boolean(bReverse);if(this.BACKW===bBackw&&(this.isAtEnd()||!this.PAUSED)){return false;}
this.BACKW=bBackw;this.PAUSED=false;if(this.onstart){this.onstart.call(this.owner,this.INDEX,this.length,this.BACKW);}
this.synchronize();return true;};Daemon.prototype.stop=function(){this.pause();if(this.onstop){this.onstop.call(this.owner,this.INDEX,this.length,this.BACKW);}
delete this.INDEX;delete this.BACKW;delete this.reversals;};"use strict";Daemon.safe=function(){Daemon.apply(this,arguments);};Daemon.safe.blank=function(){};Daemon.safe.context=Daemon.safe.prototype;Daemon.TO_BE_DEFINED=Daemon.safe.blank.prototype;Daemon.safe.blank.prototype=Daemon.safe.prototype=new Daemon.blank();Daemon.safe.prototype.constructor=Daemon.safe;Daemon.TO_BE_DEFINED.constructor=Daemon.safe.context.constructor=Object;Daemon.safe.forceCall=function(oDmn){oDmn.INDEX+=oDmn.BACKW?-1:1;var
bBreak=oDmn.task.call(oDmn.owner,oDmn.INDEX,oDmn.length,oDmn.BACKW)===false,bEnd=oDmn.isAtEnd(),bInvert=oDmn.reversals>0;if(bEnd&&!bInvert||bBreak){oDmn.PAUSED=true;return false;}
if(bEnd&&bInvert){oDmn.BACKW=!oDmn.BACKW;oDmn.reversals--;}
oDmn.synchronize();return true;};Daemon.safe.construct=Daemon.construct;Daemon.safe.buildAround=Daemon.buildAround;Daemon.safe.incorporate=Daemon.incorporate;Daemon.safe.prototype.synchronize=function(){if(this.PAUSED){return;}
clearTimeout(this.SESSION);this.SESSION=setTimeout(Daemon.safe.forceCall,this.rate,this);};Daemon.safe.prototype.pause=function(){clearTimeout(this.SESSION);this.PAUSED=true;};"use strict";Daemon.prototype.syncStart=function(bReverse){this.synchronize();if(this.start(bReverse||false)){Daemon.forceCall(this);return true;}
return false;};Daemon.prototype.play=function(bSync){return this[bSync?"start":"syncStart"]();};Daemon.prototype.reversePlay=function(bSync){return this[bSync?"start":"syncStart"](true);};Daemon.prototype.move=function(bSync,nCycles,bDirection){if(arguments.length>1&&!isNaN(nCycles)){this.reversals=Number(nCycles);}
this[bSync?"start":"syncStart"](arguments.length>2?bDirection:this.isAtEnd()!==this.BACKW);};Daemon.prototype.turn=function(bSync){this[bSync?"start":"syncStart"](!this.BACKW);};Daemon.prototype.makeLoop=function(){this.reversals=Infinity;};Daemon.prototype.unmakeLoop=function(){this.reversals=0;};Daemon.prototype.setRate=function(vTo){var nRate=Number(vTo);if(!isFinite(nRate)||nRate<1){return;}
...
var nIdx=0,nLen=Math.round(Math.abs(nHowMany)),bContinue=true,bBackw=nHowMany>0===Boolean(bReverse);this.BACKW=bBackw;for(nIdx;nIdx<nLen&&bContinue;nIdx++){if(this.BACKW===bBackw&&this.isAtEnd()){if(this.reversals>0){this.BACKW=bBackw=!this.BACKW;}
else{break;}}
bContinue=Daemon.forceCall(this)||bForce;}
return nIdx===nLen;};Daemon.prototype.skipTo=function(nIdx,bForce){if(nIdx===this.INDEX){return;}
if(isFinite(this.length)&&nIdx<0||nIdx>this.length){return;}
var
bDirection=(this.INDEX!==0&&this.INDEX!==this.length)===this.BACKW,bSuccess=this.makeSteps(this.INDEX-nIdx,true,bForce);if(this.INDEX!==0&&this.INDEX!==this.length){this.BACKW=bDirection;}
return bSuccess;};Daemon.prototype.skipFor=function(nDelta,bForce){return this.makeSteps(nDelta,this.isAtEnd()!==this.BACKW,bForce);};Daemon.prototype.close=function(bReverse,bForce){if(!isFinite(this.length)){return false;}
this.pause();this.reversals=0;return this.makeSteps(bReverse?this.INDEX:this.length-this.INDEX,bReverse,bForce);};Daemon.prototype.reclose=function(bForce){return this.close(this.isAtEnd()!==this.BACKW,bForce||false);};Daemon.prototype.restart=function(){this.stop();this.start();};Daemon.prototype.loopUntil=function(vDate){if(!isFinite(this.length)){return;}
var nTime=vDate.constructor===Date?vDate.getTime():isNaN(vDate)?Date.parse(vDate):vDate;if(isFinite(nTime)&&nTime>Date.now()){this.reversals=Math.floor((nTime-Date.now()-(this.BACKW?this.INDEX:this.length-this.INDEX)*this.rate)/(this.length*this.rate));this.start(this.isAtEnd()!==this.BACKW);}};Daemon.prototype.spread=function(nTime){if(!isFinite(this.length)){throw new TypeError("Daemon.prototype.spread - the length is not a finite number. Use Daemon.prototype.adaptLength().");}
if(nTime>0){this.rate=Math.floor(nTime/this.length);this.synchronize();}
return this.rate;};Daemon.prototype.adaptLength=function(nTime){this.length=Math.floor(nTime/this.rate);return this.length;};Daemon.prototype.playUntil=function(vDate){var nTime=vDate.constructor===Date?vDate.getTime():isNaN(vDate)?Date.parse(vDate):vDate;if(isFinite(nTime)&&nTime>Date.now()){this.length=Math.floor((nTime-Date.now())/this.rate)+this.INDEX;this.pause();this.start();}
return this.length;};(function(global,factory){if(typeof module==="object"&&typeof module.exports==="object"){module.exports=global.document?factory(global,true):function(w){if(!w.document){throw new Error("jQuery requires a window with a document");}
...
return jQuery;}));(function(factory){if(typeof define==="function"&&define.amd){define([""],factory);}else{factory(jQuery);}}(function($){var widget_uuid=0,widget_slice=Array.prototype.slice;$.cleanData=(function(orig){return function(elems){var events,elem,i;for(i=0;(elem=elems[i])!=null;i++){try{events=$._data(elem,"events");if(events&&events.remove){$(elem).triggerHandler("remove");}}catch(e){}}
...
jQuery.cssHooks[hook].set(fx.elem,fx.start.transition(fx.end,fx.pos));};});};color.hook(stepHooks);jQuery.cssHooks.borderColor={expand:function(value){var expanded={};each(["Top","Right","Bottom","Left"],function(i,part){expanded["border"+part+"Color"]=value;});return expanded;}};colors=jQuery.Color.names={aqua:"#00ffff",black:"#000000",blue:"#0000ff",fuchsia:"#ff00ff",gray:"#808080",green:"#008000",lime:"#00ff00",maroon:"#800000",navy:"#000080",olive:"#808000",purple:"#800080",red:"#ff0000",silver:"#c0c0c0",teal:"#008080",white:"#ffffff",yellow:"#ffff00",transparent:[null,null,null,0],_default:"#ffffff"};})(jQuery);var bezier=function(x1,y1,x2,y2,epsilon){var curveX=function(t){var v=1-t;return 3*v*v*t*x1+3*v*t*t*x2+t*t*t;};var curveY=function(t){var v=1-t;return 3*v*v*t*y1+3*v*t*t*y2+t*t*t;};var derivativeCurveX=function(t){var v=1-t;return 3*(2*(t-1)*t+v*v)*x1+3*(-t*t*t+2*v*t)*x2;};return function(t){var x=t,t0,t1,t2,x2,d2,i;for(t2=x,i=0;i<8;i++){x2=curveX(t2)-x;if(Math.abs(x2)<epsilon)return curveY(t2);d2=derivativeCurveX(t2);if(Math.abs(d2)<1e-6)break;t2=t2-x2/d2;}
...
return curveY(t2);};};(function(){var cache={};var ctx=null,usingWebAudio=true,noAudio=false;try{if(typeof AudioContext!=='undefined'){ctx=new AudioContext();}else if(typeof webkitAudioContext!=='undefined'){ctx=new webkitAudioContext();}else{usingWebAudio=false;}}catch(e){usingWebAudio=false;}
...
if(typeof window!=='undefined'){window.Howler=Howler;window.Howl=Howl;}})();(function($){$.widget("metro.buttonset",{version:"1.0.0",options:{click:function(btn,on){}},_buttons:{},_create:function(){var element=this.element;this._buttons=element.find("button, .button");this.init();},init:function(){var that=this;this._buttons.each(function(){var btn=$(this);btn.on('click',function(e){e.preventDefault();btn.toggleClass("active");that.options.click(btn,btn.hasClass("active"));that._trigger("click",null,{button:btn,on:(btn.hasClass("active"))});});});},_destroy:function(){},_setOption:function(key,value){this._super('_setOption',key,value);}})})(jQuery);(function($){$.widget("metro.buttongroup",{version:"1.0.0",options:{click:function(btn,on){}},_buttons:{},_create:function(){var element=this.element;this._buttons=element.find("button, .button");this.init();},init:function(){var that=this;this._buttons.each(function(){var btn=$(this);btn.on('click',function(e){e.preventDefault();that._buttons.removeClass("active");btn.addClass("active");that.options.click(btn,btn.hasClass("active"));that._trigger("click",null,{button:btn,on:(btn.hasClass("active"))});});});},_destroy:function(){},_setOption:function(key,value){this._super('_setOption',key,value);}})})(jQuery);(function($){$.widget("metro.slider",{version:"1.0.2",options:{position:0,accuracy:0,color:'default',completeColor:'default',markerColor:'default',colors:[],showHint:false,change:function(value,slider){},changed:function(value,slider){},min:0,max:100,animate:true,_slider:{vertical:false,offset:0,length:0,marker:0,ppp:0,start:0,stop:0}},_create:function(){var that=this,element=this.element,o=this.options,s=this.options._slider;if(element.data('accuracy')!=undefined){o.accuracy=element.data('accuracy')>0?element.data('accuracy'):0;}
if(element.data('animate')!=undefined){o.animate=element.data('animate');}
if(element.data('min')!=undefined){o.min=element.data('min');}
o.min=o.min<0?0:o.min;o.min=o.min>o.max?o.max:o.min;if(element.data('max')!=undefined){o.max=element.data('max');}
o.max=o.max>100?100:o.max;o.max=o.max<o.min?o.min:o.max;if(element.data('position')!=undefined){o.position=this._correctValue(element.data('position')>this.options.min?(element.data('position')>this.options.max?this.options.max:element.data('position')):this.options.min);}
...
s.vertical=element.hasClass("vertical");this._createSlider();this._initPoints();this._placeMarker(o.position);addTouchEvents(element[0]);element.children('.marker').on('mousedown',function(e){e.preventDefault();that._startMoveMarker(e);});element.on('mousedown',function(e){e.preventDefault();that._startMoveMarker(e);});},_startMoveMarker:function(e){var element=this.element,o=this.options,that=this,hint=element.children('.hint');$(document).mousemove(function(event){that._movingMarker(event);element.trigger('changed',that.options.position);if(!element.hasClass('permanent-hint')){hint.css('display','block');}});$(document).mouseup(function(){$(document).off('mousemove');$(document).off('mouseup');element.data('value',that.options.position);element.trigger('changed',that.options.position);o.changed(that.options.position,element);if(!element.hasClass('permanent-hint')){hint.css('display','none');}});this._initPoints();this._movingMarker(e)},_movingMarker:function(event){var cursorPos,percents,valuePix,vertical=this.options._slider.vertical,sliderOffset=this.options._slider.offset,sliderStart=this.options._slider.start,sliderEnd=this.options._slider.stop,sliderLength=this.options._slider.length,markerSize=this.options._slider.marker;if(vertical){cursorPos=event.pageY-sliderOffset;}else{cursorPos=event.pageX-sliderOffset;}
...
percents=this._pixToPerc(valuePix);this._placeMarker(percents);this.options.position=percents;this.options.change(Math.round(percents),this.element);},_placeMarker:function(value){var size,size2,o=this.options,colorParts=0,colorIndex=0,colorDelta=0,marker=this.element.children('.marker'),complete=this.element.children('.complete'),hint=this.element.children('.hint'),oldPos=this._percToPix(this.options.position);colorParts=o.colors.length;colorDelta=o._slider.length/colorParts;if(this.options._slider.vertical){var oldSize=this._percToPix(this.options.position)+this.options._slider.marker,oldSize2=this.options._slider.length-oldSize;size=this._percToPix(value)+this.options._slider.marker;size2=this.options._slider.length-size;this._animate(marker.css('top',oldSize2),{top:size2});this._animate(complete.css('height',oldSize),{height:size});if(colorParts){colorIndex=Math.round(size/colorDelta)-1;complete.css('background-color',o.colors[colorIndex<0?0:colorIndex]);}
if(o.showHint){hint.html(Math.round(value)).css('top',size2-hint.height()/2);}}else{size=this._percToPix(value);this._animate(marker.css('left',oldPos),{left:size});this._animate(complete.css('width',oldPos),{width:size});if(colorParts){colorIndex=Math.round(size/colorDelta)-1;complete.css('background-color',o.colors[colorIndex<0?0:colorIndex]);}
if(o.showHint){this._animate(hint.html(Math.round(value)).css('left',oldPos-hint.width()/2),{left:size-hint.width()/2});}}},_animate:function(obj,val){if(this.options.animate){obj.stop(true).animate(val);}else{obj.css(val);}},_pixToPerc:function(valuePix){var valuePerc;valuePerc=valuePix*this.options._slider.ppp;return this._correctValue(valuePerc);},_percToPix:function(value){if(this.options._slider.ppp===0){return 0;}
return value/this.options._slider.ppp;},_correctValue:function(value){var accuracy=this.options.accuracy;var max=this.options.max;var min=this.options.min;if(accuracy===0){return value;}
...
if(!n){return;}
bindings=this._bindings.slice();this._shouldPropagate=true;do{n--;}while(bindings[n]&&this._shouldPropagate&&bindings[n].execute(paramsArr)!==false);},forget:function(){this._prevParams=null;},dispose:function(){this.removeAll();delete this._bindings;delete this._prevParams;},toString:function(){return'[Signal active:'+this.active+' numListeners:'+this.getNumListeners()+']';}};var signals=Signal;signals.Signal=Signal;if(typeof define==='function'&&define.amd){define(function(){return signals;});}else if(typeof module!=='undefined'&&module.exports){module.exports=signals;}else{global['signals']=signals;}}(this));(function(global){"use strict";global.AudioView=function(metronome,timer){var scope=this;var volume=80;var beatsPerMeasure=4;var beatCount=0;var audioEnabled=true;this._tick=this.load('audio/tick0');this._tock=this.load('audio/tock3');this._timerEnd=this.load('audio/timerEnd');timer.Ended.add(function(){scope._timerEnd.volume(volume/100);scope._timerEnd.play();});metronome.Ticked.add(function(){if(null===scope._tick||!audioEnabled){return;}
var tick=scope._tick;beatCount=(beatCount+1)%beatsPerMeasure;if(beatCount%beatsPerMeasure==1){tick=scope._tock;}
tick.volume(volume/100);tick.play();});$('#audio-enabled').click(function(event){audioEnabled=event.target.checked;});$('#audio-volume').on('changed',function(event){var value=$('#audio-volume').slider('value');volume=value/100;$('#audio-label').text(value);});$('.audio-tick-button').click(function(event){var attributes=event.target.attributes;for(var i=0,len=attributes.length;i<len;i++){var name=attributes[i].name;var value=attributes[i].value;if(name==='sound-name'){scope._tick=scope.load(value);break;}}});$('.audio-tock-button').click(function(event){var attributes=event.target.attributes;for(var i=0,len=attributes.length;i<len;i++){var name=attributes[i].name;var value=attributes[i].value;if(name==='sound-name'){scope._tock=scope.load(value);break;}}});$('.sig-button').click(function(event){var attributes=event.target.attributes;for(var i=0,len=attributes.length;i<len;i++){var name=attributes[i].name;var value=attributes[i].value;if(name==='sig-value'){var timeSig=value.split('/');if(2===timeSig.length)
{beatsPerMeasure=parseInt(timeSig[0],10);metronome.noteValue=parseInt(timeSig[1],10);return;}}}});};global.AudioView.prototype={constructor:global.AudioView,load:function(soundName){return new Howl({urls:[soundName+'.mp3',soundName+'.ogg',soundName+'.wav']});}};})(this);(function(global){"use strict";global.Easing={Linear:{None:function(k){return k;}},Quadratic:{In:function(k){return k*k;},Out:function(k){return k*(2-k);},InOut:function(k){if((k*=2)<1){return 0.5*k*k;}
...
return 0.5*(Math.sqrt(1-(k-=2)*k)+1);}},Elastic:{In:function(k){var s,a=0.1,p=0.4;if(k===0){return 0;}
if(k===1){return 1;}
if(!a||a<1)
{a=1;s=p/4;}
else{s=p*Math.asin(1/a)/(2*Math.PI);}
return-(a*Math.pow(2,10*(k-=1))*Math.sin((k-s)*(2*Math.PI)/p));},Out:function(k){var s,a=0.1,p=0.4;if(k===0){return 0;}
if(k===1){return 1;}
if(!a||a<1)
{a=1;s=p/4;}
else{s=p*Math.asin(1/a)/(2*Math.PI);}
return(a*Math.pow(2,-10*k)*Math.sin((k-s)*(2*Math.PI)/p)+1);},InOut:function(k){var s,a=0.1,p=0.4;if(k===0){return 0;}
if(k===1){return 1;}
if(!a||a<1)
{a=1;s=p/4;}
else{s=p*Math.asin(1/a)/(2*Math.PI);}
if((k*=2)<1){return-0.5*(a*Math.pow(2,10*(k-=1))*Math.sin((k-s)*(2*Math.PI)/p));}
return a*Math.pow(2,-10*(k-=1))*Math.sin((k-s)*(2*Math.PI)/p)*0.5+1;}},Back:{In:function(k){var s=1.70158;return k*k*((s+1)*k-s);},Out:function(k){var s=1.70158;return--k*k*((s+1)*k+s)+1;},InOut:function(k){var s=1.70158*1.525;if((k*=2)<1){return 0.5*(k*k*((s+1)*k-s));}
...
return TWEEN.Easing.Bounce.Out(k*2-1)*0.5+0.5;}}};})(this);(function(global){"use strict";global.Metronome=function(){this.bpm=120;this.noteValue=4;this.Paused=new signals.Signal();this.Played=new signals.Signal();this.Ticked=new signals.Signal();this._playing=false;this._daemon=new Daemon.safe(this,this._tick,1);this._daemon.pause();this._lastTime=null;};global.Metronome.prototype={constructor:global.Metronome,isPlaying:function(){return this._playing;},play:function(){if(this._playing){return;}
this.Ticked.dispatch();this._lastTime=Date.now();this._daemon.play();this._playing=true;this.Played.dispatch();},pause:function(){if(!this._playing){return;}
this._daemon.pause();this._playing=false;this.Paused.dispatch();},_tick:function(){if(this.noteValue%4!==0){this.noteValue=4;}
var msPerTick=(1000*60)/(this.bpm*this.noteValue/4);var currentTime=Date.now();var diff=currentTime-this._lastTime;if(diff>=msPerTick){this._lastTime=this._lastTime+msPerTick;this.Ticked.dispatch();}}};})(this);(function(global){"use strict";global.PlaybackView=function(metronome){var scope=this;this.metronome=metronome;$('#playback-toggle').click(function(){if(metronome.isPlaying()){metronome.pause();}else{metronome.play();}});$('#playback-up').click(function(){metronome.bpm+=1;scope.updateBPM();});$('#playback-down').click(function(){metronome.bpm-=1;scope.updateBPM();});metronome.Paused.add(function(){var selector=$('#playback-toggle');if(selector.hasClass('icon-pause')){selector.removeClass('icon-pause');}
if(!selector.hasClass('icon-play')){selector.addClass('icon-play');}});metronome.Played.add(function(){var selector=$('#playback-toggle');if(selector.hasClass('icon-play')){selector.removeClass('icon-play');}
if(!selector.hasClass('icon-pause')){selector.addClass('icon-pause');}});metronome.Ticked.add(function(){$('#playback-icon').toggleClass('icon-thermometer');setTimeout(function(){$('#playback-icon').toggleClass('icon-thermometer');},100);scope.updateBPM();});};global.PlaybackView.prototype={constructor:global.PlaybackView,updateBPM:function(){$('#playback-bpm-label').text(Math.ceil(this.metronome.bpm).toString());}};})(this);(function(global){"use strict";global.TimerCurveView=function(metronome,timer){var scope=this;var container=document.getElementById('timer-canvas-container');var paper=new Raphael(container,'100%','100%');var width=container.offsetWidth;var height=container.offsetHeight;var bg=paper.rect(0,0,width,height);bg.attr('fill','#CCCCCC');bg.attr('stroke','#AAAAAA');bg.attr('stroke-width',3);var playhead=paper.path('m0,0l0,'+height);playhead.attr('stroke-width',5);playhead.attr('stroke','#DD1111');var curve=paper.path('M0,0'+height+'Q'+width/2+','+height+','+width+','+height);curve.attr('stroke','#000000');var percentage=0;var intervalId=0;var originalBPM=0;timer.Started.add(function(){originalBPM=metronome.bpm;intervalId=setInterval(function(){var t=timer.getProgress();playhead.transform('T'+t*width+',0');var k=1-2*Math.abs(t-0.5);var diff=originalBPM*percentage/100;metronome.bpm=originalBPM+k*diff;},1);});timer.Stopped.add(function(){clearInterval(intervalId);});$('.timer-curve-button').click(function(event){var attributes=event.target.attributes;for(var i=0,len=attributes.length;i<len;i++){var name=attributes[i].name;var value=attributes[i].value;if(name==='timer-value'){percentage=parseInt(value,10);if(0===percentage){curve.remove();curve=paper.path('M0,0'+height+'Q'+width/2+','+height+','+width+','+height);}else{curve.remove();curve=paper.path('M0,0'+height+'Q'+width/2+',-'+(percentage/50*40)+','+width+','+height);}
break;}}});};global.TimerCurveView.prototype={constructor:global.TimerCurveView};})(this);(function(global){'use strict';function parseTwoDigit(digit){if(digit<10){return'0'+digit;}
return digit;}
function parseThreeDigit(digit){if(digit<10){return'00'+digit;}
if(digit<100){return'0'+digit;}
return digit;}
function parseTime(milliseconds){var minutes=Math.floor(milliseconds/60000);milliseconds-=minutes*60000;var seconds=Math.floor(milliseconds/1000);milliseconds-=seconds*1000;return parseTwoDigit(minutes)+':'+parseTwoDigit(seconds)+':'+parseThreeDigit(milliseconds);}
global.TimerView=function(metronome){var scope=this;this.Started=new signals.Signal();this.Stopped=new signals.Signal();this.Ended=new signals.Signal();this._metronome=metronome;$('.timer-button').click(function(event){var attributes=event.target.attributes;for(var i=0,len=attributes.length;i<len;i++){var name=attributes[i].name;var value=attributes[i].value;if(name==='timer-value'){var minutes=parseFloat(value);if(0===minutes){scope.stop();}else{scope.start(minutes);}
break;}}});metronome.Paused.add(function(){scope.pause();});metronome.Played.add(function(){scope.resume();});this._startTime=null;this._pausedTime=null;this._totalTime=null;this._daemon=new Daemon.safe(this,this._tick,1);this._daemon.pause();this._timerBar=$('#timer-progress').progressbar();};global.TimerView.prototype={constructor:global.TimerView,getProgress:function(){if(null===this._startTime){return 0;}
if(null===this._pausedTime){return(Date.now()-this._startTime)/this._totalTime;}
return(this._pausedTime-this._startTime)/this._totalTime;},start:function(minutes){this._pausedTime=null;this._startTime=Date.now();this._totalTime=minutes*60000;this._daemon.play();this._metronome.play();this.Started.dispatch();},pause:function(){if(null!==this._pausedTime){return;}
if(null===this._startTime){return;}
this._daemon.pause();this._pausedTime=Date.now();},resume:function(){if(null===this._pausedTime||null===this._startTime){return;}
var duration=this._pausedTime-this._startTime;this._pausedTime=null;this._startTime=Date.now()-duration;this._daemon.play();},stop:function(){this._daemon.pause();this._startTime=null;this._pausedTime=null;this._totalTime=null;this._tick();this.Stopped.dispatch();},_tick:function(){if(null===this._startTime){this._timerBar.progressbar('value',0);$('#timer-label').text('00:00:000');}else{var timeElapsed=Date.now()-this._startTime;if(timeElapsed>this._totalTime){this.stop();this._metronome.pause();this.Ended.dispatch();}else{this._timerBar.progressbar('value',timeElapsed/this._totalTime*100);$('#timer-label').text(parseTime(this._totalTime-timeElapsed));}}}};})(this);(function(global){"use strict";global.VisualizerView=function(metronome,timer){var color='#FFFFFF';metronome.Ticked.add(function(){$('body').animate({'backgroundColor':color},50,"swing",function(){$('body').animate({'backgroundColor':'#ffffff'},100);});});timer.Ended.add(function(){$('body').animate({'backgroundColor':'#000000'},50,"swing",function(){$('body').animate({'backgroundColor':'#ffffff'},1000);});});$('.vs-button').click(function(event){color=$(event.target).css('background-color');});};global.VisualizerView.prototype={constructor:global.VisualizerView};})(this);(function(global){"use strict";var main=function(){var scope=this;this.metronome=new Metronome();var playback=new PlaybackView(this.metronome);var timer=new TimerView(this.metronome);var timerCurve=new TimerCurveView(this.metronome,timer);var visualizer=new VisualizerView(this.metronome,timer);var audio=new AudioView(this.metronome,timer);$(document).keyup(function(event){if(27===event.which){scope.metronome.pause();}});return scope;};$(document).ready(main);})(this);