...
var __buildTimestamp="140922114325-0500";"use strict";function Daemon(oOwner,fTask,nRate,nLen,fInit,fOnstart){if(!(this&&this instanceof Daemon)){return;}
...
if(!n){return;}
bindings=this._bindings.slice();this._shouldPropagate=true;do{n--;}while(bindings[n]&&this._shouldPropagate&&bindings[n].execute(paramsArr)!==false);},forget:function(){this._prevParams=null;},dispose:function(){this.removeAll();delete this._bindings;delete this._prevParams;},toString:function(){return'[Signal active:'+this.active+' numListeners:'+this.getNumListeners()+']';}};var signals=Signal;signals.Signal=Signal;if(typeof define==='function'&&define.amd){define(function(){return signals;});}else if(typeof module!=='undefined'&&module.exports){module.exports=signals;}else{global['signals']=signals;}}(this));(function(global){"use strict";global.AudioView=function(metronome){var scope=this;var volume=80;this._tick=null;this.load('audio/tick0');metronome.Ticked.add(function(){if(null===scope._tick){return;}
scope._tick.volume(volume);scope._tick.stop();scope._tick.play();});$('#audio-volume').on('changed',function(event){var value=$('#audio-volume').slider('value');volume=value/100;$('#audio-label').text(value);});$('.audio-button').click(function(event){scope._tick=null;var attributes=event.target.attributes;for(var i=0,len=attributes.length;i<len;i++){var name=attributes[i].name;var value=attributes[i].value;if(name==='sound-name'){scope.load(value);break;}}});};global.AudioView.prototype={constructor:global.AudioView,load:function(soundName){this._tick=new Howl({urls:[soundName+'.mp3',soundName+'.ogg',soundName+'.wav']});}};})(this);(function(global){"use strict";global.Easing={Linear:{None:function(k){return k;}},Quadratic:{In:function(k){return k*k;},Out:function(k){return k*(2-k);},InOut:function(k){if((k*=2)<1){return 0.5*k*k;}
...
return TWEEN.Easing.Bounce.Out(k*2-1)*0.5+0.5;}}};})(this);(function(global){"use strict";global.Metronome=function(){this.bpm=120;this.Paused=new signals.Signal();this.Played=new signals.Signal();this.Ticked=new signals.Signal();this._playing=false;this._daemon=new Daemon.safe(this,this._tick,1);this._daemon.pause();this._lastTime=null;};global.Metronome.prototype={constructor:global.Metronome,isPlaying:function(){return this._playing;},play:function(){if(this._playing){return;}
...
this._daemon.pause();this._playing=false;this.Paused.dispatch();},_tick:function(){var msPerTick=(1000*60)/this.bpm;var currentTime=Date.now();var diff=currentTime-this._lastTime;if(diff>msPerTick){this._lastTime=currentTime+diff-msPerTick;this.Ticked.dispatch();}}};})(this);(function(global){"use strict";global.PlaybackView=function(metronome){var scope=this;this.metronome=metronome;$('#playback-toggle').click(function(){if(metronome.isPlaying()){metronome.pause();}else{metronome.play();}});$('#playback-up').click(function(){metronome.bpm+=1;scope.updateBPM();});$('#playback-down').click(function(){metronome.bpm-=1;scope.updateBPM();});metronome.Paused.add(function(){var selector=$('#playback-toggle');if(selector.hasClass('icon-pause')){selector.removeClass('icon-pause');}
