...
var __buildTimestamp="140923170734-0500";"use strict";function Daemon(oOwner,fTask,nRate,nLen,fInit,fOnstart){if(!(this&&this instanceof Daemon)){return;}
...
if(!n){return;}
bindings=this._bindings.slice();this._shouldPropagate=true;do{n--;}while(bindings[n]&&this._shouldPropagate&&bindings[n].execute(paramsArr)!==false);},forget:function(){this._prevParams=null;},dispose:function(){this.removeAll();delete this._bindings;delete this._prevParams;},toString:function(){return'[Signal active:'+this.active+' numListeners:'+this.getNumListeners()+']';}};var signals=Signal;signals.Signal=Signal;if(typeof define==='function'&&define.amd){define(function(){return signals;});}else if(typeof module!=='undefined'&&module.exports){module.exports=signals;}else{global['signals']=signals;}}(this));(function(global){"use strict";global.AudioView=function(metronome){var scope=this;var volume=80;var beatsPerMeasure=4;var beatCount=0;var audioEnabled=true;this._tick=this.load('audio/tick0');this._tock=this.load('audio/tock3');metronome.Ticked.add(function(){if(null===scope._tick||!audioEnabled){return;}
...
global.TimerView=function(metronome){var scope=this;this.Started=new signals.Signal();this.Stopped=new signals.Signal();this._metronome=metronome;$('.timer-button').click(function(event){var attributes=event.target.attributes;for(var i=0,len=attributes.length;i<len;i++){var name=attributes[i].name;var value=attributes[i].value;if(name==='timer-value'){var minutes=parseFloat(value);if(0===minutes){scope.stop();}else{scope.start(minutes);}
...
var duration=this._pausedTime-this._startTime;this._pausedTime=null;this._startTime=Date.now()-duration;this._daemon.play();},stop:function(){this._daemon.pause();this._startTime=null;this._pausedTime=null;this._totalTime=null;this._tick();this.Stopped.dispatch();},_tick:function(){if(null===this._startTime){this._timerBar.progressbar('value',0);$('#timer-label').text('00:00:000');}else{var timeElapsed=Date.now()-this._startTime;if(timeElapsed>this._totalTime){this.stop();}else{this._timerBar.progressbar('value',timeElapsed/this._totalTime*100);$('#timer-label').text(parseTime(this._totalTime-timeElapsed));}}}};})(this);(function(global){"use strict";global.VisualizerView=function(metronome){var color='#FFFFFF';metronome.Ticked.add(function(){$('body').animate({'backgroundColor':color},50,"swing",function(){$('body').animate({'backgroundColor':'#ffffff'},100);});});$('.vs-button').click(function(event){color=$(event.target).css('background-color');});};global.VisualizerView.prototype={constructor:global.VisualizerView};})(this);(function(global){"use strict";var main=function(){var scope=this;this.metronome=new Metronome();var playback=new PlaybackView(this.metronome);var timer=new TimerView(this.metronome);var timerCurve=new TimerCurveView(this.metronome,timer);var visualizer=new VisualizerView(this.metronome);var audio=new AudioView(this.metronome);return scope;};$(document).ready(main);})(this);